...
function withError(
  ...
  return redirectTo(
    req,
    `/auth/signup/owner?${params.toString()}`
  )
}
...
async function cleanupNewSignup(
  sb: any,
  ownerId: string,
  schoolId?: string
) {
...
export async function POST(
  ...
  if (!url || !serviceKey) {
    return withError(
      req,
      form,
      'Signup server is not configured. Add SUPABASE_SERVICE_ROLE_KEY to .env.local and restart npm run dev.'
    )
  }
  ...
  if (
    ...
    return withError(
      ...
      form,
      isDuplicate
        ? 'This owner email already exists. Please sign in instead.'
        : message
    )
  }
  ...
  const schoolAttempts = [
    {
      ...
      slug,
      phone:
        form.school_phone || null,
      email:
        form.school_email || null,
      owner_id: ownerId,
      is_active: true,
      is_verified: false,
      settings: {
  ...
  for (
    const payload of schoolAttempts
  ) {
  ...
  if (!school?.id) {
    ...
    return withError(
      ...
      form,
      schoolError?.message ||
        'Owner was created, but the school profile could not be created.'
    )
  }
  ...
  if (profileError) {
    ...
    return withError(
      ...
      profileError.message ||
        'School was created, but the owner profile could not be linked.'
    )
  }
  ...
  if (
    ...
    const {
      ...
      .from('school_groups')
      .insert({
        name: `${school.name || form.school_name} Group`,
        primary_school_id:
          school.id,
        owner_user_id:
          ownerId,
    ...
    if (memberError) {
      ...
      return withError(
        ...
        form,
        memberError.message ||
          'The school group was created, but the primary school could not be linked.'
      )
    }
  }
