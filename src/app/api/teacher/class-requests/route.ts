...
function childName(
  ...
  return String(
    joinReq.child_full_name ||
      `${joinReq.child_first_name || ''} ${joinReq.child_last_name || ''}`
  )
    .trim()
    .replace(
      /\s+/g,
...
async function getLearnerPlan(
  ...
  /*
   * Otherwise use the school's own subscription.
   */
  if (!subscription) {
    subscription =
      await findUsableSubscription(
  ...
  /*
   * No usable subscription = Starter.
   */
  const planCode =
...
export async function PATCH(
  ...
  const teacherNote =
    typeof body.teacher_note ===
    'string'
      ? body.teacher_note.trim()
      : null
  ...
  if (
    ...
    return NextResponse.json(
      {
        error:
          'request_id and valid action required',
      },
      {
        status: 400,
      }
  ...
  /*
   * Reject does not create a learner.
   */
  if (
    action === 'reject'
  ) {
  ...
  /*
   * First check whether this learner already exists.
   * Existing learners do not consume another learner seat.
   */
  const {
    data: existingKids,
  } = await sb
  ...
  /*
   * Only enforce the learner limit when this approval
   * would actually create a new learner.
   */
  if (!child) {
    try {
      const limit =
      ...
      console.error(
        '[class-requests] learner plan limit check failed:',
        error
      )
